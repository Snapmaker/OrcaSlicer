                    ...
                    [tool_number, this, &current_layer_id](unsigned int id, const std::vector<float>& time_diffs) {
                        ...
                        */
                        if (this->m_is_XL_printer) {
                            ...
                            return GCodeWriter::set_temperature(temperature + m_delta_temperature, this->m_flavor, false, real_tool, comment);
                        }
                    },
                    // line replacer
...
void GCodeProcessor::update_slice_warnings()
{
    ...
    // A G-code that never extrudes (travel-only files, synthetic move-only fixtures)
    // legitimately reports no used filament; the per-filament checks below then no-op.
