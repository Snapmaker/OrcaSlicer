...
public:
    ...
    //BBS: add only gcode mode
    // Snapmaker: skip_toolpaths - when true no toolpath geometry is built or uploaded at all; used by
    //            the memory-warning dialog when the user chooses to continue without a preview.
