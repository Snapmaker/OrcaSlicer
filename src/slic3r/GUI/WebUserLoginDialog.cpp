...
ZUserLogin::ZUserLogin(std::shared_ptr<ICloudServiceAgent> cloud_agent)
    : wxDialog((wxWindow*) (wxGetApp().mainframe), wxID_ANY, "Snapmaker Orca"), m_cloud_agent(cloud_agent)
{
    SetBackgroundColour(*wxWHITE);

    // set the frame icon (Snapmaker branding)
    {
    ...
    if (!m_cloud_agent) {
        ...
        auto* m_message = new wxStaticText(this, wxID_ANY,
                                          _L("Cloud agent is not available. Please restart Snapmaker Orca and try again."),
