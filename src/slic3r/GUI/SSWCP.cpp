    ...
    try {
        ...
        if (!m_param_data.count("preset_name") || !m_param_data["preset_name"].is_string()) {
            ...
            Slic3r::sentryReportLog(Slic3r::SENTRY_LOG_ERROR,
                                    std::string("device_set_engine param [preset_name] is required or wrong type"), DEVICE_SET_ENGINE_ERR);
            return;
        }
        if (!m_param_data.count("nozzle_sizes") || !m_param_data["nozzle_sizes"].is_array()) {
            ...
            Slic3r::sentryReportLog(Slic3r::SENTRY_LOG_ERROR,
                                    std::string("device_set_engine param [nozzle_sizes] is required or wrong type"), DEVICE_SET_ENGINE_ERR);
            return;
        }
        if (!m_param_data.count("need_reload") || !m_param_data["need_reload"].is_boolean()) {
            ...
            Slic3r::sentryReportLog(Slic3r::SENTRY_LOG_ERROR,
                                    std::string("device_set_engine param [need_reload] is required or wrong type"), DEVICE_SET_ENGINE_ERR);
            return;
        }
        if (!m_param_data.count("clientId") || !m_param_data["clientId"].is_string()) {
            handle_general_fail(-1, "param [model_name] is required or wrong type");
...
std::unordered_set<std::string> SSWCP::m_mqtt_agent_cmd_list = {
    "sw_create_mqtt_client", "sw_mqtt_connect", "sw_mqtt_disconnect", "sw_mqtt_subscribe", "sw_mqtt_unpublish", "sw_mqtt_publish", "sw_mqtt_set_engine", "sw_add_connected_device"
};
