...
// Snapmaker Orca: the fork raises ExtruderMax past the 32 named extruders (see TriangleSelector.hpp)
// so combined physical + mixed filaments stay paintable. A state that far up chains several 0b1111
// continuation nibbles, which the cases above never reach, and it is deserialized with an explicit
// max_ebt rather than the default.
