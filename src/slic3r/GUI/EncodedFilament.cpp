...
Slic3r::FilamentColorCode* FilamentColorCodeQuery::GetFilaInfo(const wxString& fila_id, const EncodedFilamentColor& colors) const
{
...
wxString FilamentColorCodeQuery::GetFilaColorName(const wxString& fila_id, const EncodedFilamentColor& colors) const
{
        ...
        if (json_file.is_open())
        {
            ...
            for (const auto& json_data_item : json_data)
            {
                ...
                if (fila_color_type == wxString::FromUTF8("单色")) {
                    fila_color.m_color_type = EncodedFilamentColor::ColorType::SINGLE_CLR;
                } else if (fila_color_type == wxString::FromUTF8("多拼色")) {
                    fila_color.m_color_type = EncodedFilamentColor::ColorType::MULTI_CLR;
...
FilamentColorCode::FilamentColorCode(const wxString& color_code, FilamentColorCodes* owner, EncodedFilamentColor&& color, std::unordered_map<wxString, wxString>&& name_map)
...
Slic3r::FilamentColorCode* FilamentColorCodes::GetColorCode(const EncodedFilamentColor& colors) const
{
