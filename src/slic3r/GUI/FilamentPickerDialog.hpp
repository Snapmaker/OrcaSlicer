...
{
public:
    FilamentPickerDialog(wxWindow *parent, const wxString &fila_id, const EncodedFilamentColor &fila_color, const std::string &fila_type);
    virtual ~FilamentPickerDialog();

    // Public interface methods
    bool IsDataLoaded() const { return m_is_data_loaded; }
    wxColour GetSelectedColour() const;
    const EncodedFilamentColor& GetSelectedFilamentColor() const { return m_cur_filament_color; }
...
private:
    // UI creation methods
    wxBoxSizer* CreatePreviewPanel(const EncodedFilamentColor& fila_color, const std::string& fila_type);
    ...
    void CreateColorBitmap(const EncodedFilamentColor& fila_color);
    wxBoxSizer* CreateInfoSection();
    void SetupLabelsContent(const EncodedFilamentColor& fila_color, const std::string& fila_type);
    ...
    EncodedFilamentColor m_cur_filament_color;
