...
public:
	...
	Http& form_add_file(const std::string &name, const boost::filesystem::path &path, boost::nowide::ifstream::off_type offset = 0, size_t length = 0);
    ...
    Http& form_add_file(const std::wstring& name, const boost::filesystem::path& path, boost::nowide::ifstream::off_type offset = 0, size_t length = 0);
	// Same as above except also override the file's filename with a custom one
	Http& form_add_file(const std::string &name, const boost::filesystem::path &path, const std::string &filename, boost::nowide::ifstream::off_type offset = 0, size_t length = 0);
