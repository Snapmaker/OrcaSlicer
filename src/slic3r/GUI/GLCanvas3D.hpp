...
public:
    ...
    // Snapmaker: reset_volumes() must know whether it runs during canvas destruction so it can
    // avoid touching the (already dying) sidebar/selection. See Selection::clear(notify_sidebar).
...
public:
    ...
    void render_thumbnail(ThumbnailData &           thumbnail_data,
                                 ...
                                 bool                      for_picking  = false,
                                 bool                      ban_light    = false);
    // Snapmaker: overload taking an explicit extruder-colour vector (mixed-filament previews
    // render with substituted colours instead of the project palette).
    void render_thumbnail(ThumbnailData& thumbnail_data, unsigned int w, unsigned int h, const ThumbnailsParams& thumbnail_params,
                                 const GLVolumeCollection &volumes,
                                 std::vector<ColorRGBA>&   extruder_colors,
                                 Camera::EType             camera_type,
                                 ...
                                 bool                      for_picking  = false,
                                 bool                      ban_light    = false);
    // Snapmaker named-viewpoint overload. The 8th param is ThumbnailView (scoped enum) vs the
    // Camera::ViewAngleType of the overload above — overload resolution picks this one only
    // when a ThumbnailView is passed, so existing call sites are unaffected.
    // ThumbnailView maps 1:1 onto Camera::ViewAngleType; ThumbnailView::Top maps to
    // Camera::ViewAngleType::Top (unified named-view framing), NOT to the legacy plate-centred
    // Camera::ViewAngleType::Top_Plate. See ThumbnailView.hpp for the framing implications.
    void render_thumbnail(ThumbnailData& thumbnail_data, unsigned int w, unsigned int h, const ThumbnailsParams& thumbnail_params,
                                 const GLVolumeCollection &volumes,
                                 std::vector<ColorRGBA>&   extruder_colors,
                                 Camera::EType             camera_type,
    ...
    // Snapmaker: trailing skip_toolpaths lets the preview be loaded without building the
    // toolpath geometry (mixed-filament / Local-Z previews only need the shells + legend).
