...
int GLVolumeCollection::load_real_wipe_tower_preview(
    ...
    bool render_brim, float rotation_angle, bool size_unknown, bool opengl_initialized)
{
    int plate_idx = obj_idx - 1000;
    ...
    // Merge wipe tower and brim meshes
    TriangleMesh mesh = wt_mesh;
    if (render_brim) {
        mesh.merge(brim_mesh);
    }

    // Load the actual mesh into the first color slot
    if (!colors.empty()) {
        v.model_per_colors[0].init_from(mesh);
    }

    // Create convex hull for collision detection and raycasting
