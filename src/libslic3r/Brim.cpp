    ...
    std::vector<unsigned int>& printExtruders)
{
    ...
    for (unsigned int extruderNo : printExtruders) {
        ...
        for (const auto& objectWithExtruder : objPrintVec) {
            ...
            Polygons           holes_object;
            Polygons           holes_support;
            const bool extruder_matches  = (objectWithExtruder.second == extruderNo);
            const bool should_generate   = use_brim_ears
                ? brimToWrite.at(object->id()).obj            // btPainted: brim generation does not depend on extruder matching
                : (extruder_matches && brimToWrite.at(object->id()).obj);
            if (should_generate) {
