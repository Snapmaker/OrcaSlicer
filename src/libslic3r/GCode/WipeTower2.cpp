...
WipeTower::ToolChangeResult WipeTower2::finish_layer()
{
    ...
    if (loops_num > 0) {
        ...
        for (int i = 0; i < loops_num; ++i) {
            // SM Orca: FIX - Copy Bambu Studio's approach to prevent visual artifacts
            // The issue: ClipperLib's jtMiter switches to DoSquare() when miter limit is exceeded
            // This causes point count to drop (e.g., 336->256) creating "two extra lines" artifact
            // Bambu's solution: Use default offset + apply simplify() to smooth geometry
            // The simplification removes redundant points and prevents visual discontinuities

            // Use default offset (jtMiter with DefaultMiterLimit=3, matching Bambu)
            poly = offset(poly, scale_(spacing)).front();

            // CRITICAL: Apply Bambu's simplification approach
            // Convert to Polyline and apply Douglas-Peucker simplification
            // This removes redundant points and smooths geometric transitions
            Polyline pl = to_polyline(poly);

            // Use simplified tolerance matching Bambu's WT_SIMPLIFY_TOLERANCE_SCALED
            // Bambu: 0.001 / SCALING_FACTOR, we use scaled(0.01) for similar effect
            pl.simplify(scaled<double>(0.01));

            // Find closest point and travel there
            // Note: pl.points has one extra point (closed loop), so we iterate to size()-1
            int cp = 0;
            Point current_pos = Point::new_scale(writer.x(), writer.y());
            float min_dist_sq = std::numeric_limits<float>::max();

            for (size_t j = 0; j < pl.points.size() - 1; ++j) {
                float dist_sq = (pl.points[j].cast<float>() - current_pos.cast<float>()).squaredNorm();
