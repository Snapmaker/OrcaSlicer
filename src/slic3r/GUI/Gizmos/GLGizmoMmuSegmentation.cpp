...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (size_t extruder_idx = 0; extruder_idx < n_extruder_colors; ++extruder_idx) {
        ...
        if (draw_color_button(
            int(extruder_idx) + 1,                   // idx (doubles as the visible label)
            "###extruder_color_",                    // button_id
            extruder_color,                          // color
            extruder_color,                          // mapped_color (not used in here)
...
void GLGizmoMmuSegmentation::render_filament_remap_ui(float window_width, float max_tooltip_width, float scale)
{
    ...
    // Snapmaker: the remap grid is indexed by the sidebar's logical filament order; resolve a
    // display slot to the colour of the filament (physical or mixed) it stands for.
    auto display_color = [this](size_t display_idx) -> ColorRGBA {
        if (display_idx >= m_display_filament_ids.size())
            return ColorRGBA();
        const unsigned int filament_id = m_display_filament_ids[display_idx];
        if (filament_id == 0 || filament_id > m_extruders_colors.size())
            return ColorRGBA();
        return m_extruders_colors[filament_id - 1];
    };
    ...
    for (size_t src = 0; src < n_extr; ++src) {
        const unsigned int src_filament_id = m_display_filament_ids[src];
        if (src_filament_id == 0 || src_filament_id > m_extruders_colors.size())
            continue;
        // ORCA: skip filaments that are not used by this object
        if (m_used_filaments.find(size_t(src_filament_id - 1)) == m_used_filaments.end())
            continue;
        ...
        ColorRGBA src_color = display_color(src);
        ColorRGBA map_color = display_color(m_extruder_remap[src]);

        bool src_clicked = draw_color_button(
            (int)src + 1,                       // idx
            "###remap_src_",                    // button_id
            src_color,                          // color
            map_color,                          // mapped_color (shows bubble if not matches with Color)
        ...
        if (ImGui::BeginPopup(pop_id.c_str())) {
            ...
            for (int dst = 0; dst < (int)n_extr; ++dst) {
                const unsigned int dst_filament_id = m_display_filament_ids[dst];
                if (dst_filament_id == 0 || dst_filament_id > m_extruders_colors.size())
                    continue;

                if (popup_displayed > 0 && (popup_displayed % max_per_line != 0))
                     ImGui::SameLine();

                ColorRGBA dst_color = display_color(size_t(dst));
                bool dst_clicked = draw_color_button(
                    dst + 1,                      // idx
                    "###remap_dst_",              // button_id
                    dst_color,                    // color
                    dst_color,                    // mapped_color (non fuctional in here)
...
void GLGizmoMmuSegmentation::remap_filament_assignments()
{
    ...
    for (size_t src = 0; src < n_extr; ++src) {
        ...
        // Snapmaker: remap operates on the sidebar's logical filament order, so translate the
        // display slots back to painting states (= actual filament ids) before building the map.
        ...
        // ORCA: state 0 (unpainted) is no longer remapped here - the base volume extruder is
        // remapped explicitly below.
