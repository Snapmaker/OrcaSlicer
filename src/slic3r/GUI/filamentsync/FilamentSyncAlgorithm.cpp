...
{
    const size_t design_count  = design_colors.size();
    const size_t machine_count = machine_colors.size();

    std::vector<int> result(design_count, -1);
    if (design_count == 0 || machine_count == 0)
        return result;

    // Precompute Lab values for all machine filaments
    std::vector<Lab> machine_lab(machine_count);
    size_t j = 0;
    for (const auto& m : machine_colors) {
        rgb_to_lab(m.m_color_r, m.m_color_g, m.m_color_b,
                   machine_lab[j].L, machine_lab[j].a, machine_lab[j].b);
        ++j;
    }

    // For each design filament, find the perceptually closest machine filament
    size_t i = 0;
    for (const auto& d : design_colors) {
        float designL, designA, designB;
        rgb_to_lab(d.m_color_r, d.m_color_g, d.m_color_b,
                   designL, designA, designB);

        float bestDist = std::numeric_limits<float>::max();
        int   bestIdx  = -1;
        for (size_t mj = 0; mj < machine_count; ++mj) {
            float dL   = designL - machine_lab[mj].L;
            float dA   = designA - machine_lab[mj].a;
            float dB   = designB - machine_lab[mj].b;
            float dist = dL * dL + dA * dA + dB * dB; // squared Euclidean (avoids sqrt)
            if (dist < bestDist) {
                bestDist = dist;
                bestIdx  = static_cast<int>(mj);
            }
        }
        result[i] = bestIdx;
        ++i;
