...
def check_filament_compatible_printers(vendor, vendor_folder):
    ...
    for file_path in vendor_path.rglob("*.json"):
        ...
        if 'name' not in data:
            # Auxiliary data files (e.g. Snapmaker filament_compatibility.json)
            # live alongside preset JSONs but are not presets themselves.
            continue
...
# formula.
# Snapmaker keeps its authoritative legacy ids (fork OTA/preset identity).
SETTING_ID_FORMULA_EXEMPT_VENDORS = {"BBL", "Snapmaker"}
