...
std::unique_ptr<NetworkAgent> create_agent_from_config(const std::string& log_dir, AppConfig* app_config)
{
    ...
    // Create BBL cloud agent for Bambu printers
    auto cloud_agent = NetworkAgentFactory::create_cloud_agent("bbl", log_dir);
    if (!cloud_agent) {
        BOOST_LOG_TRIVIAL(error) << "Failed to create BBL cloud agent";
    }
