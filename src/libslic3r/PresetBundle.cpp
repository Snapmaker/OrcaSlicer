    ...
    const std::set<std::string>* retain_configs)
{
        ...
        !entry.setting_id.empty())
    {
        ...
        // Applied here rather than in parse_subfile so it also covers entries restored from the
        // vendor preset cache, and so it keeps taking precedence over the inherits lookup below.
        filament_id = entry.setting_id;
        // BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
        //                            << ": missing filament_id for " << preset_name
        //                            << ", falling back to setting_id " << entry.setting_id;
    }
