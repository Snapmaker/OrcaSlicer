...
//    const coordf_t link_max_length_factor = 3.;
    ...
    // Pre-compute contact zone positions for support top contact speed split.
    // Position: 0 = not in top contact zone, 1 = first (furthest from model / lowest Z in block),
    //           2 = middle, 3 = top (closest to model / TopContact layer).
    // A "top contact zone block" is a contiguous run of TopInterface layers in the
    // support_layers array, optionally capped by a TopContact layer at the highest Z.
    // Block boundaries are determined by layer_type semantics, not by Z-distance heuristics.
    std::vector<int> contact_zone_position(support_layers.size(), 0);
    if (get_value_at(config, config.support_top_contact_speed_split, ConfigFlowDomain::Process)) {
        // Step 1: Mark TopContact layers → position 3
        size_t tc_idx = 0;
        for (size_t si = 0; si < support_layers.size(); ++si) {
            coordf_t z = support_layers[si]->print_z;
            while (tc_idx < top_contacts.size() && top_contacts[tc_idx]->print_z < z - EPSILON)
                ++tc_idx;
            if (tc_idx < top_contacts.size() && std::abs(top_contacts[tc_idx]->print_z - z) < EPSILON) {
                contact_zone_position[si] = 3;
                ++tc_idx;
            }
        }
        // Step 2-3: Scan interface_layers for contiguous TopInterface blocks,
        // then map each block entry back to the corresponding support_layer.
        // This approach works even when merge_remove_empty drops some
        // TopInterface entries (empty polygons), because the block is
        // contiguous in the interface_layers array itself.
        size_t si_idx = 0;
        for (size_t ii = 0; ii < interface_layers.size(); ) {
            // Find start of a TopInterface block
            while (ii < interface_layers.size() &&
                   interface_layers[ii]->layer_type != SupporLayerType::TopInterface)
                ++ii;
            if (ii >= interface_layers.size()) break;

            size_t block_start = ii;
            while (ii < interface_layers.size() &&
                   interface_layers[ii]->layer_type == SupporLayerType::TopInterface)
                ++ii;
            size_t block_end = ii;

            // Map each block entry to the matching support_layer by print_z
            for (size_t bi = block_start; bi < block_end; ++bi) {
                coordf_t z = interface_layers[bi]->print_z;
                while (si_idx < support_layers.size() && support_layers[si_idx]->print_z < z - EPSILON)
                    ++si_idx;
                if (si_idx < support_layers.size() &&
                    contact_zone_position[si_idx] == 0 &&
                    std::abs(support_layers[si_idx]->print_z - z) < EPSILON) {
                    contact_zone_position[si_idx] = (bi == block_start) ? 1 : 2;
                    ++si_idx;
                }
            }
        }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        [&config, &slicing_params, &support_params, &support_layers, &bottom_contacts, &top_contacts, &intermediate_layers, &interface_layers, &base_interface_layers, &layer_caches, &loop_interface_processor,
            &bbox_object, &angles, &interface_angles, n_raft_layers, link_max_length_factor, &contact_zone_position]
            (const tbb::blocked_range<size_t>& range) {
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id)
        {
            ...
            if (config.support_interface_top_layers == 0) {
                ...
                // Support top contact speed split: apply per-layer role to contact loops
                // generated by loop_interface_processor (they default to erSupportMaterialInterface).
                if (get_value_at(config, config.support_top_contact_speed_split, ConfigFlowDomain::Process)) {
                    ExtrusionRole target_role = ExtrusionRole::erSupportMaterialInterface;
                    if (contact_zone_position[support_layer_id] == 1)
                        target_role = ExtrusionRole::erSupportMaterialInterfaceFirst;
                    else if (contact_zone_position[support_layer_id] == 2)
                        target_role = ExtrusionRole::erSupportMaterialInterfaceMiddle;
                    else if (contact_zone_position[support_layer_id] == 3)
                        target_role = ExtrusionRole::erSupportMaterialInterfaceTop;
                    if (target_role != ExtrusionRole::erSupportMaterialInterface) {
                        for (ExtrusionEntity *ee : top_contact_layer.extrusions) {
                            if (auto *path = dynamic_cast<ExtrusionPath*>(ee); path &&
                                path->role() == ExtrusionRole::erSupportMaterialInterface)
                                path->set_extrusion_role(target_role);
                        }
                    }
                }
            ...
            auto extrude_interface = [&](SupportGeneratorLayerExtruded &layer_ex, InterfaceLayerType interface_layer_type) {
                if (! layer_ex.empty() && ! layer_ex.polygons_to_extrude().empty()) {
                    ...
                    // Snapmaker: Generate perimeter loop before interface infill for better surface connection.
                    // Applies to true interface layers (not InterfaceAsBase or raft contact).
                    if (!interface_as_base && !raft_contact && config.support_transition_perimeter.value) {
                        ...
                        if (!perimeter_loops.empty()) {
                            // Support top contact speed split: apply per-layer role to perimeter loops
                            ExtrusionRole loop_role = ExtrusionRole::erSupportMaterialInterface;
                            if (get_value_at(config, config.support_top_contact_speed_split, ConfigFlowDomain::Process) &&
                                (interface_layer_type == InterfaceLayerType::TopContact ||
                                 interface_layer_type == InterfaceLayerType::Interface)) {
                                if (contact_zone_position[support_layer_id] == 1)
                                    loop_role = ExtrusionRole::erSupportMaterialInterfaceFirst;
                                else if (contact_zone_position[support_layer_id] == 2)
                                    loop_role = ExtrusionRole::erSupportMaterialInterfaceMiddle;
                                else if (contact_zone_position[support_layer_id] == 3)
                                    loop_role = ExtrusionRole::erSupportMaterialInterfaceTop;
                            }
                            extrusion_entities_append_loops(
                                layer_ex.extrusions,
                                std::move(perimeter_loops),
                                loop_role,
                                float(interface_flow.mm3_per_mm()),
                                float(interface_flow.width()),
                                float(interface_flow.height()));
                        }
                    ...
                    if (!interface_areas.empty()) {
                        fill_expolygons_generate_paths(
                            ...
                            interface_as_base ? ExtrusionRole::erSupportMaterial :
                                // Support top contact speed split: assign per-layer role
                                (!raft_contact && get_value_at(config, config.support_top_contact_speed_split, ConfigFlowDomain::Process) &&
                                 (interface_layer_type == InterfaceLayerType::TopContact || interface_layer_type == InterfaceLayerType::Interface)) ?
                                    (contact_zone_position[support_layer_id] == 1 ? ExtrusionRole::erSupportMaterialInterfaceFirst :
                                     contact_zone_position[support_layer_id] == 2 ? ExtrusionRole::erSupportMaterialInterfaceMiddle :
                                     contact_zone_position[support_layer_id] == 3 ? ExtrusionRole::erSupportMaterialInterfaceTop :
            ...
            if ( ! base_interface_layer.empty() && ! base_interface_layer.polygons_to_extrude().empty()) {
                ...
                filler->angle           = base_interface_layer.layer->up ? interface_angles[(support_layer_id + 1) % interface_angles.size()] + M_PI_2 :
                                                                           (angles[support_layer_id == 0 ? 0 : (support_layer_id - 1) % angles.size()] + M_PI_2);
