    ...
    for (int layer_nr = 0; layer_nr < m_object->layer_count(); layer_nr++) {
        ...
        if (layer_nr < blockers.size()) {
            ...
            for (auto& wt : layer->loverhangs_with_type) {
                ...
                for (auto& d : diffed) filtered.emplace_back(d, wt.second);
            }
        ...
        if (support_critical_regions_only && is_auto(stype)) {
            ...
            if (lower_layer == nullptr || enforcers.empty() || layer_nr >= enforcers.size() || enforcers[layer_nr].empty()) {
                ...
                ExPolygons enforced_overhangs_scr = offset_ex(to_expolygons(enforcers[layer_nr]), enforcer_overhang_offset);
