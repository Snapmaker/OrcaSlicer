...
FilamentPickerDialog::FilamentPickerDialog(wxWindow *parent, const wxString& fila_id, const EncodedFilamentColor& fila_color, const std::string& fila_type)
...
wxBoxSizer* FilamentPickerDialog::CreatePreviewPanel(const EncodedFilamentColor& fila_color, const std::string& fila_type)
{
...
void FilamentPickerDialog::CreateColorBitmap(const EncodedFilamentColor &fila_color)
{
    ...
    if (fila_color.ColorCount() > 0) {
        ...
        wxBitmap init_bmp = create_filament_bitmap(wx_colors, COLOR_DEMO_SIZE,
                                                fila_color.m_color_type == EncodedFilamentColor::ColorType::GRADIENT_CLR);
...
void FilamentPickerDialog::SetupLabelsContent(const EncodedFilamentColor &fila_color, const std::string &fila_type)
{
...
wxScrolledWindow* FilamentPickerDialog::CreateColorGrid()
{
    ...
    if (!color_map->empty()) {
        for (const auto& color_pair : *color_map) {
            const EncodedFilamentColor& fila_color = color_pair.first;        // color info
...
{
    EncodedFilamentColor fila_color = color_code.GetFilaColor();
    ...
    if (m_more_btn) {
        m_more_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
            ...
            if (result.GetColour() != original_data.GetColour()) {
                ...
                m_cur_filament_color.m_color_type = EncodedFilamentColor::ColorType::SINGLE_CLR;
