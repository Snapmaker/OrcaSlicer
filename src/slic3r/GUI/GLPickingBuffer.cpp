...
{
    return _framebuffer != 0 && _colorTexture != 0 && _depthRenderbuffer != 0 &&
           _width > 0 && _height > 0 &&
...
void GLPickingBuffer::EndRender()
{
    ...
    BindFramebuffer(GL_DRAW_FRAMEBUFFER,
                    static_cast<GLuint>(_previousDrawFramebuffer));
    BindFramebuffer(GL_READ_FRAMEBUFFER,
                    static_cast<GLuint>(_previousReadFramebuffer));
    _renderBound = false;
}
...
bool GLPickingBuffer::ReadPoint(int x, int y, PointSample& sample) const
{
    ...
    glsafe(::glReadPixels(x, y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                          sample.color.rgba.data()));
    glsafe(::glReadPixels(x, y, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT,
                          &sample.depth));
    return true;
}

bool GLPickingBuffer::ReadColorRect(int x, int y, int width, int height,
                                    std::vector<ColorPixel>& pixels) const
{
    ...
    glsafe(::glReadPixels(x, y, width, height, GL_RGBA, GL_UNSIGNED_BYTE,
                          pixels.data()));
    return true;
}
...
bool GLPickingBuffer::CreateResources(int width, int height)
{
    ...
    glsafe(::glGetIntegerv(
        _framebufferType == OpenGLManager::EFramebufferType::Arb
            ? GL_RENDERBUFFER_BINDING
            : GL_RENDERBUFFER_BINDING_EXT,
        &previousRenderbuffer));

    Slic3r::ScopeGuard restoreBindings([this, previousDrawFramebuffer,
                                         previousReadFramebuffer, previousTexture,
                                         previousRenderbuffer]() {
        if (_framebufferType == OpenGLManager::EFramebufferType::Arb)
            glsafe(::glBindRenderbuffer(GL_RENDERBUFFER,
                                        static_cast<GLuint>(previousRenderbuffer)));
        else if (_framebufferType == OpenGLManager::EFramebufferType::Ext)
            glsafe(::glBindRenderbufferEXT(
                GL_RENDERBUFFER_EXT, static_cast<GLuint>(previousRenderbuffer)));
    ...
    glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0,
                          GL_RGBA, GL_UNSIGNED_BYTE, nullptr));
    glsafe(::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
    glsafe(::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST));

    if (_framebufferType == OpenGLManager::EFramebufferType::Arb) {
        glsafe(::glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
        ...
        glsafe(::glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT,
                                       width, height));
        glsafe(::glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                           GL_RENDERBUFFER, _depthRenderbuffer));
    } else {
        glsafe(::glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
        ...
        glsafe(::glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT,
                                          GL_DEPTH_COMPONENT, width, height));
        glsafe(::glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT,
...
void GLPickingBuffer::BindFramebuffer(GLenum target, GLuint framebuffer) const
{
    if (_framebufferType == OpenGLManager::EFramebufferType::Arb)
        ...
        glsafe(::glBindFramebufferEXT(target, framebuffer));
}

void GLPickingBuffer::GetFramebufferBindings(GLint& drawFramebuffer,
                                             GLint& readFramebuffer) const
{
    if (_framebufferType == OpenGLManager::EFramebufferType::Ext) {
        glsafe(::glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING_EXT,
                               &drawFramebuffer));
        glsafe(::glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING_EXT,
                               &readFramebuffer));
    } else {
