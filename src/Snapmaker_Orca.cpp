...
int CLI::run(int argc, char **argv)
{
    ...
    auto check_plate_wipe_tower = [get_print_sequence, is_smooth_timelapse, new_extruder_count](Slic3r::GUI::PartPlate* plate, int plate_index, DynamicPrintConfig& print_config, plate_obj_size_info_t &plate_obj_size_info) {
        ...
        BOOST_LOG_TRIVIAL(debug) << "check_plate_wipe_tower estimate input"
                                 ...
                                 << " extruder_count=" << new_extruder_count
        ...
        for (size_t i = 0; i < plate_count; i++)
        {
            ...
            if (assemble_plate.need_arrange)
            {
                ...
                if (!arrange_cfg.is_seq_print && (assemble_plate.filaments_count > 1)||(enable_wrapping_detect && !current_wrapping_exclude_area.empty()))
                {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("prime_tower_width %1% wipe_tower_rotation_angle %2% prime_volume %3%, rib_wall %4%, brim_chamfer %5%, brim_chamfer_max_width %6%") % width_option->value % rotation_angle_option->value % volume_option->value %prime_tower_rib_wall_option->value %brim_chamfer_option->value %brim_chamfer_max_width_option->value;
...
extern "C" {
    __declspec(dllexport) int __stdcall Snapmaker_Orca_main(int argc, wchar_t **argv)
    {
        ...
        // (SET_DEFULTER_HANDLER). We deliberately do not: initSentry() has already installed
