...
// New method: Check ALL moves including Travel moves (not just extrusion paths)
// This fixes the critical vulnerability where Travel moves were not validated
bool BuildVolume::all_moves_inside(const GCodeProcessorResult& paths, const BoundingBoxf3& paths_bbox, bool ignore_bottom) const
{
    // Validate all significant moves (Extrude and Travel), excluding Retract/Unretract
    auto move_significant = [](const GCodeProcessorResult::MoveVertex &move) {
        return move.type == EMoveType::Extrude || move.type == EMoveType::Travel;
    };
    static constexpr const double epsilon = BedEpsilon;

    // Debug: count violations and log first few
    int violation_count = 0;
    const int max_log_violations = 5;
    ...
    case BuildVolume_Type::Rectangle:
    {
        ...
        for (size_t i = 0; i < paths.moves.size(); ++i) {
            const auto& move = paths.moves[i];
            if (move_significant(move) && !build_volume.contains(move.position.cast<double>())) {
                violation_count++;
                if (violation_count <= max_log_violations) {
                    const char* type_str = (move.type == EMoveType::Travel) ? "Travel" : "Extrude";
                    BOOST_LOG_TRIVIAL(warning) << "Move #" << i << " (" << type_str << ") outside bounds: "
                        << "pos=(" << move.position.x() << ", " << move.position.y() << ", " << move.position.z() << "), "
                        << "bounds=[" << build_volume.min.x() << "-" << build_volume.max.x() << ", "
                        << build_volume.min.y() << "-" << build_volume.max.y() << ", "
                        << build_volume.min.z() << "-" << build_volume.max.z() << "]";
                }
            }
        }

        if (violation_count > max_log_violations) {
            BOOST_LOG_TRIVIAL(warning) << "... and " << (violation_count - max_log_violations)
                << " more move(s) outside bounds (total: " << violation_count << ")";
        }
    ...
    case BuildVolume_Type::Circle:
    {
        ...
        const float r = unscaled<double>(m_circle.radius) + epsilon;
        const float r2 = sqr(r);
        const float max_z = m_max_print_height + epsilon;
        ...
        for (size_t i = 0; i < paths.moves.size(); ++i) {
            ...
            float dist_sqr = (to_2d(move.position) - c).squaredNorm();
            bool xy_ok = dist_sqr <= r2;
            bool z_ok = (m_max_print_height == 0.0) || (move.position.z() <= max_z);

            if (!xy_ok || !z_ok) {
                violation_count++;
                if (violation_count <= max_log_violations) {
                    const char* type_str = (move.type == EMoveType::Travel) ? "Travel" : "Extrude";
                    BOOST_LOG_TRIVIAL(warning) << "Move #" << i << " (" << type_str << ") outside circle: "
                        << "pos=(" << move.position.x() << ", " << move.position.y() << ", " << move.position.z() << "), "
                        << "center=(" << c.x() << ", " << c.y() << "), "
                        << "radius=" << r << ", dist=" << std::sqrt(dist_sqr);
                }
            }
        }
    ...
    case BuildVolume_Type::Custom:
    {
        ...
        for (size_t i = 0; i < paths.moves.size(); ++i) {
            ...
            bool xy_ok = Geometry::inside_convex_polygon(m_top_bottom_convex_hull_decomposition_bed,
            ...
            if (!xy_ok || !z_ok) {
                ...
                if (violation_count <= max_log_violations) {
                    const char* type_str = (move.type == EMoveType::Travel) ? "Travel" : "Extrude";
                    BOOST_LOG_TRIVIAL(warning) << "Move #" << i << " (" << type_str << ") outside convex polygon: "
                        << "pos=(" << move.position.x() << ", " << move.position.y() << ", " << move.position.z() << ")";
                }
            }
        }
