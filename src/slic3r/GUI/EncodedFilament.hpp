...
public:
    bool operator<(const EncodedFilamentColor& other) const { 
...
{
    bool operator()(const EncodedFilamentColor& lhs, const EncodedFilamentColor& rhs) const noexcept { return lhs < rhs; }
};
using FilamentColor2CodeMap = std::map<EncodedFilamentColor, FilamentColorCode*, EncodedFilaColorEqual>;
...
public:
    ...
    wxString GetFilaColorName(const wxString& fila_id, const EncodedFilamentColor& colors) const;

private:
    FilamentColorCode* GetFilaInfo(const wxString& fila_id, const EncodedFilamentColor& colors) const;
...
public:
    ...
    FilamentColorCode* GetColorCode(const EncodedFilamentColor& colors) const;
...
public:
    ...
    FilamentColorCode(const wxString& color_code, FilamentColorCodes* owner, EncodedFilamentColor&& color, std::unordered_map<wxString, wxString>&& name_map);
    ~FilamentColorCode() {};
...
public:
    ...
    EncodedFilamentColor    GetFilaColor() const { return m_fila_color; }
...
private:
    ...
    EncodedFilamentColor                          m_fila_color;
