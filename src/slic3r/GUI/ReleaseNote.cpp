    ...
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe), wxID_ANY, _L("Network plug-in update"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    operation_tips = new ::Label(this, Label::Body_12, _L("Click OK to update the Network plug-in now. If a file is in use, the update will be applied the next time Snapmaker Orca launches."), LB_AUTO_WRAP);
    ...
    : DPIDialog(parent, wxID_ANY, _L("New version of Snapmaker Orca"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
    ...
    auto sm    = create_scaled_bitmap("Snapmaker_Orca", nullptr, 64);
	...
	// ORCA: the release note is now rendered from markdown into html_source and pushed with
	// SetPage() from the NAVIGATING handler above, so the local releasenote.html is not loaded here.
	// fs::path ph(data_dir());
...
void UpdateVersionDialog::update_version_info(wxString release_note, wxString version)
{
    ...
    if (is_running_in_msix())
        m_text_up_info->SetLabel(wxString::Format(_L("New version available: %s. Please update Snapmaker Orca from the Microsoft Store."), version));
    else
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    comfirm_before_enter_text = _L("1. Please confirm Snapmaker Orca and your printer are in the same LAN.");
