...
// Flush dataset type — controls which lookup table is used.
// 0 = StandardFlow (flush_data_standard.txt)
// 1 = HighFlow    (flush_data_highflow.txt)
enum class FlushDataset : int {
    StandardFlow = 0,
    HighFlow     = 1,
};
...
public:
    // flush_dataset: FlushDataset enum value (0=StandardFlow, 1=HighFlow)
    explicit GenericFlushPredictor(const int flush_dataset);
