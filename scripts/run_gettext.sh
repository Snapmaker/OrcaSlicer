...
if $FULL_MODE; then
    ...
    if $has_sources; then
        ...
        generated_pot_file="${generated_i18n_dir}/Snapmaker_Orca.pot"
...
do
    ...
    if [ -f "$dir/Snapmaker_Orca_${lang}.po" ]; then
        if $FULL_MODE && [ -f "$pot_file" ]; then
            merged_po=$(mktemp)
            if ! msgmerge -N --no-wrap -o "$merged_po" "$dir/Snapmaker_Orca_${lang}.po" "$pot_file"; then
                echo "Error encountered with msgmerge command for language ${lang}."
                rm -f "$merged_po"
                exit 1
            fi

            if files_equal_ignoring_pot_date "$dir/Snapmaker_Orca_${lang}.po" "$merged_po"; then
                rm -f "$merged_po"
            else
                mv "$merged_po" "$dir/Snapmaker_Orca_${lang}.po"
            fi
        fi
