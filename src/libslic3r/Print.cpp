...
StringObjectException Print::validate(StringObjectException *warning, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
...

	{
        ...
        // ORCA: append per-extruder layer height / support nozzle diagnostics to the single warning out-param.
        auto warn = [warning](const std::string &msg, const std::string &opt_key, const PrintObject *object) {
            if (warning) {
                if (! warning->string.empty())
                    warning->string += "\n";
                warning->string += msg;
                warning->opt_key = opt_key;
