...
// Snapmaker: Snapmaker_VERSION (used by the toolpath OBJ exporter). Included explicitly so this
// translation unit also builds with SLIC3R_PCH=OFF, where pchheader.hpp is not pulled in.
                ...
                bool skip_toolpaths)
{
    ...
    // Snapmaker: when the user answered the memory warning with "continue without preview" we must not
    // build/upload the toolpath geometry at all. Everything else (statistics, sliders, shells, bed) is
    // still set up so the preview page stays usable. m_no_render_path makes the renderer skip the paths.
    m_no_render_path = skip_toolpaths;
    if (skip_toolpaths)
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(": skip_toolpaths=true, no toolpath geometry is built, moves=%1%") % gcode_result.moves.size();

    // convert data from PrusaSlicer format to libvgcode format
    libvgcode::GCodeInputData data;
    if (!skip_toolpaths)
        data = libvgcode::convert(gcode_result, str_tool_colors, str_color_print_colors, m_viewer);
...
// #endif // !VGCODE_ENABLE_COG_AND_TOOL_MARKERS
    ...
    if (skip_toolpaths) {
        // Snapmaker: the viewer holds no vertices, so derive the bounding box straight from the moves.
        // all_paths_inside() is deliberately not called here: it walks every move again (page faults on
        // huge results) and m_contained_in_bed already defaults to true, so no false warning is shown.
...
void GCodeViewer::render_all_plates_stats(const std::vector<const GCodeProcessorResult*>& gcode_result_list, bool show /*= true*/) const {
    ...
    auto get_used_filament_from_volume = [this, imperial_units, &filament_diameters, &filament_densities](double volume, int extruder_id) {
        // Snapmaker: guard against stale extruder ids outliving a filament-count change
    ...
    // title and item data
    {
        ...
        {
            // Snapmaker: a plate can lose its slice result between the render call and here
    ...
    // item
    {
        ...
        for (auto it = model_volume_of_extruders_all_plates.begin(); it != model_volume_of_extruders_all_plates.end(); it++) {
            if (i < model_used_filaments_m_all_plates.size() && i < model_used_filaments_g_all_plates.size()) {
                ...
                // Snapmaker: filament_colors comes from the plater config and may be shorter than the
                // per-extruder statistics maps; fall back to the first color instead of reading out of range.
                const size_t color_idx = (it->first >= 0 && static_cast<size_t>(it->first) < filament_colors.size()) ?
                                             static_cast<size_t>(it->first) : 0;
                if (!filament_colors.empty())
                    append_item(filament_colors[color_idx], columns_offsets);
            }
            i++;
        }
