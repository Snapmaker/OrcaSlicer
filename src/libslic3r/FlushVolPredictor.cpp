    ...
    auto rgb_hex_to_dec = [](const std::string& hexstr, FlushPredict::RGBColor& color)->bool
        {
            ...
            int cr = hexToByte(hexstr.substr(1, 2));
            int cg = hexToByte(hexstr.substr(3, 2));
            int cb = hexToByte(hexstr.substr(5, 2));
            if (cr < 0 || cg < 0 || cb < 0) {
                color.r = 0, color.g = 0, color.b = 0;
                return false;
            }
            color.r = (unsigned char)cr;
            color.g = (unsigned char)cg;
            color.b = (unsigned char)cb;
            return true;
        };

    std::ifstream in(data_file);
    if (!in.is_open()) {
        BOOST_LOG_TRIVIAL(warning) << "FlushVolPredictor: failed to open data file: " << data_file;
        m_valid = false;
        return;
    }
    std::string line;
...
static std::mutex predictor_mutex;

static const std::unordered_map<int, std::string> s_dataset_files = {
    {0, "/flush/flush_data_standard.txt"},
    {1, "/flush/flush_data_highflow.txt"},
};

GenericFlushPredictor::GenericFlushPredictor(const int flush_dataset)
{
    {
        std::lock_guard<std::mutex> lock(predictor_mutex);
        auto iter = predictor_instances.find(flush_dataset);
        if (iter != predictor_instances.end()) {
            predictor = &iter->second;
            return;
        }
    }
    auto fit = s_dataset_files.find(flush_dataset);
    if (fit == s_dataset_files.end()) {
        predictor = nullptr;
        return;
    }
    std::string path = Slic3r::resources_dir() + fit->second;
    FlushVolPredictor new_predictor(path);
    {
        std::lock_guard<std::mutex> lock(predictor_mutex);
        auto [it, inserted] = predictor_instances.try_emplace(flush_dataset, std::move(new_predictor));
        predictor = &it->second;
    }
}
